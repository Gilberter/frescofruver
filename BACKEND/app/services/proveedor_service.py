...
def _proveedor_activo(prov: Proveedor) -> bool:
    return (prov.estado or "").strip().lower() == "activo"
...
def perfil_proveedor(db: Session, proveedor_id: int) -> ProveedorPerfil:
    ...
    base = ProveedorOut.model_validate(prov)
...
def recibir_orden(db: Session, orden_id: int, actor_id: int) -> OrdenCompra:
    ...
    if orden.estado_orden != EstadoOrden.pendiente:
        raise HTTPException(
            status.HTTP_422_UNPROCESSABLE_ENTITY,
            detail="Solo se pueden recibir órdenes en estado Pendiente",
        )
    ...
    orden = crud_proveedor.update_estado_orden(db, orden, EstadoOrden.completada)
    crud_auditoria.registrar(db, "recibir_orden", f"Orden id={orden_id} marcada como Completada", actor_id)
    return orden
